...
class ImageCreateView(CreateAPIView):
    ...
    def get(self, request, format=None):
        ...
        for x in self.df2_nifty_CE.index:
            ...
            elif(((self.location_ohlcv("close", x) - buying_price) * lot_size <= -500) or ((self.location_ohlcv("close", x) - buying_price) * lot_size >= 1500) or (self.location_index(x) >= time_index(x, "15:00:00"))) and (not sell_flag):
        ...
        self.df2_nifty_CE['profit'] = (self.df2_nifty_CE['profit']) * lot_size
        self.df2_nifty_CE["mtm"] = mtm
        self.df2_nifty_CE["cost"] = (self.df2_nifty_CE["placed_order"].multiply(
            self.df2_nifty_CE["close"])) * lot_size
